...
Thanks for taking our coding challenge! We'd like for you to implement a basic event management system, 
...
We provided you with some HTML (index.html) to start as well as some Spring boilerplate for a backend server, 
...
Steps to complete:
1) OK. Create a Venue table to match the schema defined by Venue.java
2) OK. Modify the events table to contain a venueId column
3) OK. Create a venue when the app starts - Wrigley Field - and make sure that venue is linked to 
   the White Sox vs. Cubs event
...
The frontend of the app should show the list of events in the database on page load and 
then dynamically update when a new event is added.
...
You may want to leverage an IDE to develop and debug. The application will be available on localhost:8080.
